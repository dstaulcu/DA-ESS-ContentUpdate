...
class DnsTwistCommand(GeneratingCommand):
    ...
    .. code-block::
        | inputlookup tweets | countmatches fieldname=word_count text
    """

    domainlist_file_name = Option(
        doc='''**Syntax:** **csv=***<path>*
        **Description:** CSV file from which repeated random samples will be
        drawn''',
        name='domainlist', require=False)#, validate=validators.File())

    def generate(self):
        event_count = 0
        csv_file_names = []

        csv_file_names.append(make_splunkhome_path(['etc', 'apps', 'Splunk_SA_CIM', 'lookups', 'cim_corporate_email_domains.csv']))
        csv_file_names.append(make_splunkhome_path(['etc', 'apps', 'Splunk_SA_CIM', 'lookups', 'cim_corporate_web_domains.csv']))
        ...
        for domain_to_twist in domains_to_twist:
            ...
            for domain in domains:
                ...
                if domain['domain-name'] in domain_to_twist:
                    continue
                event_count += 1
                yield {'_time': time.time(), 'event_no': event_count, '_raw': '*'+domain['domain-name'], 'domain': '*'+domain['domain-name']+'*'}
